package com.pengji.linker.gencode.entity.cols;
...
public @interface SupCol {
    ...
    /**
     * 是否隐藏, true - 隐藏() default ""; false - 显示() default ""; absHide 或 absTrue
     * - 绝对隐藏，不会被鼠标右键菜单选择() default ""; absShow 或 absFalse - 绝对显示，不会被鼠标右键菜单选择()
     * default "";
     */
    ...
    /**
     * 是否允许被粘贴 ,默认：supcan.xml true - 允许() default ""; false - 不允许() default "";
     * supcan.xml - 通常是允许，但是当列不可编辑时(包括隐藏时)，将以supcan.xml中的
     * <pasteAbleWhenUnEditAble> 的设定为准() default "";
     */
    ...
    /**
     * 列宽 整数 - 绝对宽度 (单位:像素数)() default ""; 百分比 - 窗口宽度的百分比, 如：20%() default "";
     * 为小于1的分数 - 比例因子，用于分配剩余的宽度, 如0.2() default ""; fitHeader 或 header -
     * 自动伸展到能使表头标题能够完整显示() default "";
     */
    ...
    /**
     * 列标题旁边的小图标 可以是图标的URL, 也可以是如下格式的串: url=[?]() default "";pos=[?]
     * pos用于指定图标的位置, 是水平(left/right)、 垂直(top/middle/bottom)方向的组合. 举例如下:
     * url=../ac.png() default "";pos=right,bottom 建议采用png或ico这类透明的图片
     */
    ...
    /**
     * 下拉列(droplis, droptreelist)单元格的文字显示方式 0 - 仅显示文字部分() default ""; 1 -
     * 仅显示键值(即key)部分() default ""; 2 - 键值+" - "+文字() default "";
     */
    String dropDisplayType() default "";

    /**
     * 虚拟列的数据分隔符 串，比如"/", 如果设了这个串，在加载数据后，程序将按这个分隔符自动对各行数据进行处理，使其看上去呈现多列的效果。此外,
     * 各个段中纵向、横向如果有连续相同的数据，将自动呈现纵、横的合并效果 (无) 注1: 仅改变显示效果，并不改变数据() default "";
     * 注2: VColSep列有诸多限制，例如只能用于string型、不能作为树的排序列等等() default "";
     */
    String VColSep() default "";

    /**
     * VColSepStyle 虚拟列的合并方式 row - 只允许跨行的垂直合并() default ""; col - 只允许跨列的横向合并()
     * default ""; row,col - 跨行、跨列自动合并() default "";
     */
